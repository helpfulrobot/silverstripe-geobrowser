...
class BrowseSuburbPage extends BrowseAbstractPage {
	...
	public static function create_suburb($googleMapAddressArray, BrowseCitiesPage $parent) {
		if($parent && isset($googleMapAddressArray["LocalityName"]) && isset($googleMapAddressArray["Longitude"]) && isset($googleMapAddressArray["Latitude"])) {
			...
			if($name) {
				...
				if(!$page) {
					...
					$page->MetaTitle = $name;
					$page->MenuTitle = $name;
					$page->Longitude = $googleMapAddressArray["Longitude"];
					$page->Latitude = $googleMapAddressArray["Latitude"];
